...
var jq = (() => {
    ...
    return (async function (moduleArg = {}) {
        ...
        const runtimeInitPromise = new Promise(resolve => { runtimeInitResolve = resolve; });
        const encoder = new TextEncoder;
        const decoder = new TextDecoder;
        function toByteArray(str) { return encoder.encode(str); }
        function fromCharCodes(charCodes) { return decoder.decode(new Uint8Array(charCodes)); }
        function executeJq(args) { const stackBefore = stackSave(); const preExitCode = typeof process !== "undefined" ? process.exitCode : undefined; stdoutBuffer.length = 0; stderrBuffer.length = 0; let exitCode; try {
        ...
        } return { stdout: fromCharCodes(stdoutBuffer).trim(), stderr: fromCharCodes(stderrBuffer).trim(), exitCode }; }
        function runJq(jsonString, query, flags) { stdinBuffer = toByteArray(jsonString); if (!flags.includes("-M")) {
            flags = ["-M", ...flags];
        } const args = [...flags, query, "/dev/stdin"]; return executeJq(args); }
        function runJqVersion() { const result = executeJq(["--version"]); if (result.stderr) {
            throw new Error(result.stderr);
        } return result.stdout; }
        Module = { ...Module, noInitialRun: true, noExitRuntime: true, onRuntimeInitialized() { runtimeInitResolve(); }, printErr(message) { if (typeof message === "string" && message.includes("program exited (with status:")) {
                return;
            } console.error(message); }, preRun() { FS.init(() => { if (stdinBuffer.length === 0)
                return null; const byte = stdinBuffer[0]; stdinBuffer = stdinBuffer.slice(1); return byte !== null && byte !== void 0 ? byte : null; }, charCode => { if (charCode != null)
                stdoutBuffer.push(charCode); }, charCode => { if (charCode != null)
                stderrBuffer.push(charCode); }); }, raw(jsonString, query, flags = []) { return runtimeInitPromise.then(() => runJq(jsonString, query, flags)); }, version() { return runtimeInitPromise.then(() => runJqVersion()); } };
        var arguments_ = [];
        ...
        function initRuntime() { runtimeInitialized = true; if (!Module["noFSInit"] && !FS.initialized)
            FS.init(); TTY.init(); wasmExports["F"](); FS.ignorePermissions = false; }
        function preMain() { }
        ...
        var wasmBinaryFile;
        function findWasmBinary() { return locateFile("jq.wasm"); }
        ...
        async function createWasm() { function receiveInstance(instance, module) { wasmExports = instance.exports; wasmMemory = wasmExports["E"]; updateMemoryViews(); wasmTable = wasmExports["H"]; removeRunDependency("wasm-instantiate"); return wasmExports; } addRunDependency("wasm-instantiate"); function receiveInstantiationResult(result) { return receiveInstance(result["instance"]); } var info = getWasmImports(); if (Module["instantiateWasm"]) {
            return new Promise((resolve, reject) => { Module["instantiateWasm"](info, (mod, inst) => { resolve(receiveInstance(mod, inst)); }); });
        } wasmBinaryFile !== null && wasmBinaryFile !== void 0 ? wasmBinaryFile : (wasmBinaryFile = findWasmBinary()); try {
        ...
        var FS_createPreloadedFile = (parent, name, url, canRead, canWrite, onload, onerror, dontCreateFile, canOwn, preFinish) => { var fullname = name ? PATH_FS.resolve(PATH.join2(parent, name)) : parent; var dep = getUniqueRunDependency(`cp ${fullname}`); function processData(byteArray) { function finish(byteArray) { preFinish === null || preFinish === void 0 ? void 0 : preFinish(); if (!dontCreateFile) {
            FS_createDataFile(parent, name, byteArray, canRead, canWrite, canOwn);
        } onload === null || onload === void 0 ? void 0 : onload(); removeRunDependency(dep); } if (FS_handledByPreloadPlugin(byteArray, fullname, finish, () => { onerror === null || onerror === void 0 ? void 0 : onerror(); removeRunDependency(dep); })) {
            return;
        ...
        var _proc_exit = code => { var _a; EXITSTATUS = code; if (!keepRuntimeAlive()) {
        ...
        var wasmImports = { a: ___assert_fail, n: ___call_sighandler, e: ___syscall_fcntl64, B: ___syscall_fstat64, m: ___syscall_getcwd, h: ___syscall_ioctl, y: ___syscall_lstat64, z: ___syscall_newfstatat, f: ___syscall_openat, l: ___syscall_readlinkat, A: ___syscall_stat64, D: __abort_js, p: __emscripten_runtime_keepalive_clear, q: __gmtime_js, r: __localtime_js, s: __mktime_js, t: __timegm_js, v: __tzset_js, C: _emscripten_date_now, k: _emscripten_resize_heap, i: _environ_get, j: _environ_sizes_get, b: _exit, c: _fd_close, x: _fd_fdstat_get, g: _fd_read, w: _fd_seek, d: _fd_write, o: _proc_exit, u: _strptime };
        var wasmExports = await createWasm();
        var ___wasm_call_ctors = wasmExports["F"];
        var _main = Module["_main"] = wasmExports["G"];
        ...
        function run(args = arguments_) { if (runDependencies > 0) {
            ...
            return; initRuntime(); preMain(); readyPromiseResolve(Module); (_a = Module["onRuntimeInitialized"]) === null || _a === void 0 ? void 0 : _a.call(Module); var noInitialRun = Module["noInitialRun"] || true; if (!noInitialRun)
